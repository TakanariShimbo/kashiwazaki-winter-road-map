...
機能：タイル5種切替／場所検索／現在地（GPS・自動表示）／目的地ピン（タップで設置・町名/直線距離表示）／線スタイル設定（色・太さ・線種をカスタマイズ、localStorage保存）／回転・傾き（3D）／レイヤー切替／クリックで出典付きポップアップ。

デフォルト表示：消雪パイプ・除雪路線・緊急確保路線（歩道除雪はOFF）。
